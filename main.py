...
class Engineer: 
    ...
    def create_memfile_from_block(self, file_name: str, block: str, memfs: MemoryFS): 
        idx = file_name.rfind('/')
        if idx != -1: 
            dir_name = file_name[:idx]
            memfs.makedirs(dir_name, recreate=True)
        with memfs.open(file_name, "w") as mf: 
            mf.write(block)
        
    def create_zipfile_from_block(self, file_name: str, block:str, zipfs: ZipFS): 
        idx = file_name.rfind('/')
        if idx != -1: 
            dir_name = file_name[:idx]
            zipfs.makedirs(dir_name, recreate=True)
        with zipfs.open(file_name, "w") as zf: 
            zf.write(block) 
    ...
    def create_proj_in_mem(self, block_map: dict): 
        memfs = MemoryFS() 
        for k, v in block_map.items(): 
            self.create_memfile_from_block(k, v, memfs=memfs)
        self.compress_mem_folder(memfs=memfs)
        memfs.close() 

    def create_proj_zip(self, block_map: dict): 
        zipfs = ZipFS(f"{self.proj_name}.zip", write=True)
        for k, v in block_map.items(): 
            self.create_zipfile_from_block(k, v, zipfs=zipfs)
        zipfs.close() 

    def compress_mem_folder(self, memfs: MemoryFS): 
        zipfs = ZipFS(f"{self.proj_name}.zip", write=True) 
        fs.copy.copy_fs(memfs, zipfs)
        zipfs.close() 
...
def run():
    ...
    eng.create_proj_zip(block_map=block_map)
